...
This guide covers:
- **Contributing** new network topologies via Pull Request
- **Self-hosting** your own Mock Meraki API instance

## Self-Hosting Model

**This is a self-hosting project.** Community topologies are NOT deployed to the hosted demo instance (`meraki-api.highvelocitynetworking.com`). That instance is maintained separately for Infoblox Universal DDI integration demos.

When you contribute a topology:
1. Create a Pull Request with your topology
2. CI validates your topology automatically
3. Maintainer reviews and approves
4. Your topology is merged to the repository
5. Anyone (including you) can deploy it to their **own AWS account**
6. The hosted demo instance remains unchanged (uses `hub_spoke` only)
...
- **Required status checks**: `validate` and `lint` must pass
- **Required reviews**: 1 approval from maintainer
- **No direct push**: All changes must go through Pull Request
...
│     └── Add your topology file                                   │
│                                                                  │
│  2. Open Pull Request                                            │
│     └── CI runs automatically                                    │
│         ├── validate (topology validation)                       │
│         └── lint (code style)                                    │
│                                                                  │
│  3. CI must pass ✓                                               │
│     └── Required for merge                                       │
...
│                                                                  │
│  5. Merge to main                                                │
│     └── Only hub_spoke.py changes auto-deploy                    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
Both `validate` and `lint` jobs **must pass** before your PR can be merged.

### Deployment

| What Changed | CI Validation | Auto-Deploy |
|--------------|---------------|-------------|
| `hub_spoke.py` | ✓ Required | ✓ Yes |
| New topology (e.g., `retail.py`) | ✓ Required | ✗ Manual only |
| Generators (`generators/*.py`) | ✓ Required | ✗ Manual only |
| `mesh.py`, `multi_org.py` | ✓ Required | ✗ Manual only |

**Why this design?**
- The hosted demo (`meraki-api.highvelocitynetworking.com`) runs `hub_spoke` topology only
...
- Contributors can self-host and deploy any topology to their own AWS

### Manual Deployment (Maintainers)

To deploy a topology manually:
